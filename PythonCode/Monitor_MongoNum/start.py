...
if __name__=='__main__':
    ...
    db = client.emaildb
    collection = db.temp
    num11 = collection.find({"create_time": {"$gt": todaydatestamp}}).count()
    ...
    sql9 = "select count(*) from fjl.resume where update_time >= '%s'" % (today_date)
    logging.info("sql10: {0}".format(sql9))
    ...
    sql12 = "select count(*) from djpendingdata where insert_date >= '{0}' and sent = '0'".format(today_date)   #今日大街待发送数目
    sql13 = "select count(*) from ycpendingdata where insert_date >= '{0}' and sent = '0'".format(today_date)   #今日英才待发送数目
